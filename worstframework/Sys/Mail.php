...
class Sys_Mail
{
    ...
    */
    public function setCRLF($crlf = "\n")
    {
        ...
        if (!defined('Sys_Mail_MIMEPart_CRLF')) {
            define('Sys_Mail_MIMEPart_CRLF', $crlf, true);
        }
    }
    ...
    */
    private function addTextPart(&$message)
    {
        ...
        if (!empty($message)) {
            ...
            $message = new Sys_Mail_MIMEPart($this->text, $params);
        }
    }
    ...
    */
    private function addHtmlPart(&$message)
    {
        ...
        if (!empty($message)) {
            ...
            $message = new Sys_Mail_MIMEPart($this->html, $params);
        }
    }
    ...
    */
    private function addMixedPart(&$message)
    {
        ...
        $message = new Sys_Mail_MIMEPart('', $params);
    }
        ...
        switch ($type) {
            ...
            case 'smtp':
                ...
                foreach ($recipients as $recipient) {
                    $addresses = Sys_Mail_RFC822::parseAddressList($recipient, $this->smtp_params['helo'], null, false);
                ...
                foreach ($this->headers as $name => $value) {
                    if ($name == 'Cc' OR $name == 'Bcc') {
                        $addresses = Sys_Mail_RFC822::parseAddressList($value, $this->smtp_params['helo'], null, false);
                        foreach ($addresses as $address) {
                            $smtp_recipients[] = sprintf('%s@%s', $address->mailbox, $address->host);
                        }
                    }
                ...
                if (isset($this->return_path)) {
                    ...
                    $from = Sys_Mail_RFC822::parseAddressList($this->headers['From']);
